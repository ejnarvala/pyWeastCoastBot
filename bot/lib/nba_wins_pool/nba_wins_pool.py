...
def build_team_breakdown_df(game_data_df, teams_df, team_id_to_owner, team_id_to_price, leaderboard_df):
    """
    ...
    """
    df = game_data_df
    nba_standings_df = pd.DataFrame({"wins": df.groupby("winning_team_id")["id"].count(),
                                     "losses": df.groupby(["losing_team_id"])["id"].count()}).fillna(0)
    ...
    team_breakdown_df = team_breakdown_df.drop(columns="id")[["owner", "full_name", "wins", "losses", "auction_price"]]
...
def build_race_plot_df(game_data_df, leaderboard_df):
    ...
    race_plot_df = pd.DataFrame(
        race_plot_counts.index.get_level_values(0).unique().insert(0, pd.to_datetime("2021-10-18", utc=True)))
    for owner in leaderboard_df.index:
        race_plot_df = race_plot_df.merge(
            race_plot_counts[:, owner].cumsum().rename(owner), how='left', left_on="date", right_on='date'
        )
